      ...
      >
        {messagesLoading ? (
          ...
          <>
            ...
            {showPending && pending && (
              <div
                className={`flex flex-col gap-0.5 ${role === 'agent' ? 'items-end' : 'items-start'}`}
              >
                {/* Not dimmed with opacity: that drops the text below AA contrast
                    (measured 4.43:1). A caption carries the "not yet sent" state. */}
                <p
                  className={`max-w-[75%] rounded-2xl px-4 py-2 text-sm ${
                ...
                <span aria-hidden="true" className="px-2 text-xs text-slate-600">
                  Sending…
                </span>
              </div>
            )}
